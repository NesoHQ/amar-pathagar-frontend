    ...
    <Layout>
      <div className="space-y-6">
          ...
          <div className="p-6">
            <div className="flex items-start gap-6">
              ...
              <div className="flex-shrink-0">
                <div className="w-32 h-32 border-4 border-old-ink bg-old-border flex items-center justify-center">
                  {user.avatar_url ? (
              ...
              <div className="flex-1">
                ...
                <p className="text-old-grey text-lg mb-4">@{user.username}</p>

                {user.bio && (
                  <p className="text-old-grey mb-4 border-l-4 border-old-ink pl-4">{user.bio}</p>
                )}

                <div className="flex flex-wrap gap-4">
                  <div className={`px-4 py-2 border-2 border-old-ink ${scoreStatus.color} font-bold uppercase text-sm`}>
                    {scoreStatus.label} Standing
                  </div>
                  {user.is_donor && (
                    <div className="px-4 py-2 border-2 border-green-600 bg-green-100 text-green-700 font-bold uppercase text-sm">
                      🎁 Donor
                    </div>
                  )}
                  {user.role === 'admin' && (
                    <div className="px-4 py-2 border-2 border-red-600 bg-red-100 text-red-700 font-bold uppercase text-sm">
                      ⚙️ Admin
                    </div>
                  )}
                </div>
              ...
              {/* Success Score */}
              <div className="flex-shrink-0 text-center border-4 border-old-ink bg-gradient-to-br from-old-ink to-gray-800 text-old-paper p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.2)]">
                <p className="text-xs uppercase tracking-widest opacity-75 mb-2">Success Score</p>
                <p className="text-5xl font-bold">{user.success_score}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="border-4 border-old-ink bg-white p-6">
            <h2 className="text-xl font-bold uppercase tracking-wider mb-4">📊 Activity</h2>
          ...
          {user.location_address && (
            <div className="border-4 border-old-ink bg-white p-6">
              <h2 className="text-xl font-bold uppercase tracking-wider mb-4">📍 Location</h2>
              <p className="text-old-grey">{user.location_address}</p>
            </div>
          )}
        </div>
      </div>
